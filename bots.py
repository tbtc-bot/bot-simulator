...
class Bot:
    ...
    def __init__(self, symbol, GO, GS, SF, OS, OF, SL):
        self.symbol = symbol
        self.GO = GO
        self.GS = GS
        self.SF = SF
    ...
    def __str__(self):
        return f"{self.symbol}_GO{self.GO}_GS{self.GS}_SF{self.SF}_OS{self.OS}_OF{self.OF}_SL{self.SL}"
    ...
    def _setTakeProfit(self, position):
        if position.size < 0:
            ...
            takeProfitPrice = position.entryPrice * (1 - self.GS / 100)
        else:
